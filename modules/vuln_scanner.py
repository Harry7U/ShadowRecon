...
def scan_vulnerabilities(active_hosts_file, urls_file, js_file, config):
    ...
    os.system(f"nuclei -list {active_hosts_file} -tags cve,osint,tech -silent -o nuclei_results.txt")
    os.system(f"cat {urls_file} | grep -E 'lfi' | nuclei -tags lfi -silent -o nuclei_lfi.txt")
